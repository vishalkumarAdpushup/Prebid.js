...
const PRIORITY_ORDER_FOR_DESKTOP_SIZES_ASC = [
	'1x1',
	'970x90',
	'970x250',
	'160x600',
...
function isMobileUserAgent() {
	return getUserAgent().match(
		/(mobile)|(ip(hone|ad))|(android)|(blackberry)|(nokia)|(phone)|(opera\smini)/i
	);
}
...
function compareSizesByPriority(size1, size2) {
	var priorityOrderForSizesAsc = isMobileDevice()
		? PRIORITY_ORDER_FOR_MOBILE_SIZES_ASC
		: PRIORITY_ORDER_FOR_DESKTOP_SIZES_ASC;
...
function getSpaces(bidRequests, ml, disableVideo) {
	...
	if (impType) {
		bidRequests = bidRequests.filter(
			(bid) =>
				...
				(impType & VAST_INSTREAM
					? !bid.mediaTypes[VIDEO].context || bid.mediaTypes[VIDEO].context == 'instream'
					: bid.mediaTypes[VIDEO].context == 'outstream')
		);
	}
	...
	es.str = Object.keys(spacesStruct)
		.map((size) =>
			spacesStruct[size]
				.map((bid, i) => {
					es.vs += getVs(bid);
...
function getViewabilityTracker() {
	...
	function intersectionCallback(entries) {
		entries.forEach(function (entry) {
			...
			if (entry.isIntersecting) {
				if (
					entry.intersectionRatio >= VIEWABILITY_MIN_RATIO &&
	...
	function processIntervalVisibilityStatus(elapsedVisibleIntervals, element, callback) {
		...
		if (visibleIntervals === TIME_PARTITIONS) {
			...
			setTimeout(
				processIntervalVisibilityStatus.bind(this, visibleIntervals, element, callback),
				VIEWABILITY_TIME / TIME_PARTITIONS
			);
		}
	}
...
function visibilityHandler(obj) {
	if (obj.div) {
		...
		getViewabilityTracker().onView(
			obj.div,
			registerAuction.bind(undefined, STORAGE_VIEW_PREFIX + obj.name)
		);
	}
}
