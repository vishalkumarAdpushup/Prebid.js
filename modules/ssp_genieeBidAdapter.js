...
function makeCommonRequestData(bid, geparameter, refererInfo) {
  const data = {
    ...
    loc:
      ...
      refererInfo?.legacy.referer ||
      encodeURIComponentIncludeSingleQuotation(
        geparameter[GEPARAMS_KEY.LOCATION]
      ) ||
      "",
    ct0:
      geparameter[GEPARAMS_KEY.GENIEE_CT0] !== "undefined"
        ? encodeURIComponentIncludeSingleQuotation(
            geparameter[GEPARAMS_KEY.GENIEE_CT0]
          )
        : "",
    referer:
      refererInfo?.ref ||
      encodeURIComponentIncludeSingleQuotation(
        geparameter[GEPARAMS_KEY.REFERRER]
      ) ||
      "",
    topframe: window.parent == window.self ? 1 : 0,
    cur: bid.params.hasOwnProperty("currency")
      ? bid.params.currency
      : DEFAULT_CURRENCY,
    requestid: bid.bidId,
  ...
  try {
    if (
      window.self.toString() !== "[object Window]" ||
      window.parent.toString() !== "[object Window]"
    ) {
      data.err = "1";
    }
  } catch (e) {}
  ...
  if (GEPARAMS_KEY.IDENTIFIER_FOR_ADVERTISERS in geparameter) {
    data.idfa = encodeURIComponentIncludeSingleQuotation(
      geparameter[GEPARAMS_KEY.IDENTIFIER_FOR_ADVERTISERS]
    );
  }
  ...
  if (hasParamsNotBlankString(geparameter, GEPARAMS_KEY.ZIP)) {
    data.zip = encodeURIComponentIncludeSingleQuotation(
      geparameter[GEPARAMS_KEY.ZIP]
    );
  }
  if (hasParamsNotBlankString(geparameter, GEPARAMS_KEY.COUNTRY)) {
    data.country = encodeURIComponentIncludeSingleQuotation(
      geparameter[GEPARAMS_KEY.COUNTRY]
    );
  }
  if (hasParamsNotBlankString(geparameter, GEPARAMS_KEY.CITY)) {
    data.city = encodeURIComponentIncludeSingleQuotation(
      geparameter[GEPARAMS_KEY.CITY]
    );
  }
  if (hasParamsNotBlankString(geparameter, GEPARAMS_KEY.LONGITUDE)) {
    data.long = encodeURIComponentIncludeSingleQuotation(
      geparameter[GEPARAMS_KEY.LONGITUDE]
    );
  }
  ...
  if (
    GEPARAMS_KEY.CUSTOM in geparameter &&
    isPlainObject(geparameter[GEPARAMS_KEY.CUSTOM])
  ) {
  ...
  if (isPlainObject(gecuparameter)) {
    if (hasParamsNotBlankString(gecuparameter, GECUPARAMS_KEY.VERSION)) {
      data.gc_ver = encodeURIComponentIncludeSingleQuotation(
        gecuparameter[GECUPARAMS_KEY.VERSION]
      );
    }
    if (hasParamsNotBlankString(gecuparameter, GECUPARAMS_KEY.MINOR_VERSION)) {
      data.gc_minor = encodeURIComponentIncludeSingleQuotation(
        gecuparameter[GECUPARAMS_KEY.MINOR_VERSION]
      );
    }
    if (hasParamsNotBlankString(gecuparameter, GECUPARAMS_KEY.VALUE)) {
      data.gc_value = encodeURIComponentIncludeSingleQuotation(
        gecuparameter[GECUPARAMS_KEY.VALUE]
      );
    }
  }
...
function makeBannerRequestData(bid, geparameter, refererInfo) {
  ...
  if (hasParamsNotBlankString(geparameter, GEPARAMS_KEY.BUNDLE)) {
    data.apid = encodeURIComponentIncludeSingleQuotation(
      geparameter[GEPARAMS_KEY.BUNDLE]
    );
  }
...
function makeBannerBidResponse(bid, request) {
  ...
  bidResponse.ad = makeBidResponseAd(
    beacon +
      "<div>" +
      makeChangeHeightEventMarkup(request) +
      decodeURIComponent(bid.adm) +
      "</div>"
  );
...
function makeChangeHeightEventMarkup(request) {
  return (
    '<script>window.addEventListener("load",function(){window.parent.document.getElementById("' +
    request.bid.adUnitCode +
...
export const spec = {
  ...
  isBidRequestValid: function (bidRequest) {
    ...
    if (
      ...
      ALLOWED_CURRENCIES.indexOf(currencyType) === -1
    ) {
