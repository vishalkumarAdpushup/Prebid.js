...
function vdoIsBidResponseValid(vdoresponse) {
	if (
		!vdoresponse.requestId ||
		!vdoresponse.cpm ||
		!vdoresponse.creativeId ||
		!vdoresponse.ttl ||
		!vdoresponse.currency ||
		!vdoresponse.meta.advertiserDomains
	) {
		return false;
	}
...
export const spec = {
	...
	buildRequests: (vdoValidBidRequests, bidderRequest) => {
		...
		const placements = groupBy(
			vdoValidBidRequests.map((bidRequest) => vdoBuildPlacement(bidRequest)),
			'host'
		);
		return Object.keys(placements).map((host) =>
			vdoBuildRequest(
				winTop,
				host,
				placements[host].map((placement) => placement.adUnit),
				bidderRequest
			)
		);
	},
	...
	onBidWon: (vdobid) => {
		...
		if (vdobid.nurl !== '') {
			vdobid.nurl = vdobid.nurl.replace(/\$\{AUCTION_PRICE\}/, cpm);
	...
	getUserSyncs: (userSyncOptions, vdoServerResponses, userGdprConsent, UserUspConsent) => {
		...
		for (let i = 0; i < vdoServerResponses.length; i++) {
			...
			const vdoImgSync =
				serverResponseHeaders != null && userSyncOptions.pixelEnabled
					? serverResponseHeaders.get('X-PLL-UserSync-Image')
					: null;
			const vdoIframeSync =
				serverResponseHeaders != null && userSyncOptions.iframeEnabled
					? serverResponseHeaders.get('X-PLL-UserSync-Iframe')
					: null;
		...
		return [
			allIframeSyncs.filter(uniques).map((it) => {
				return { type: 'iframe', url: it };
			}),
			allImageSyncs.filter(uniques).map((it) => {
				return { type: 'image', url: it };
			})
		]
			.reduce(flatten, [])
			.filter(uniques);
